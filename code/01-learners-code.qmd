...
gta_valid |> 
  ...
  ggplot(aes(variety, difference_seeds, colour = variety)) + 
  geom_hline(yintercept = 0, linetype = 2) +
  ggtitle("GTAMix1") +
  geom_jitter() + 
  geom_text(aes(label = workstation), position = position_jitter(0.1, 0.1), size = 2, colour = "black") + 
  theme(
    legend.position = "none"
  )
```
...
```{r}
difference_in_seed_by_sample <- function(sample_name){
  gta_valid |> 
  filter(str_detect(sample_code, "Mix")) |> 
  select(workstation, sample_code, obs_id, variety, ends_with("seeds")) |> 
  mutate(difference_seeds = expected_seeds - predicted_seeds) |> 
  ...
  ggtitle(sample_name) +
  geom_jitter() + 
  geom_text(aes(label = workstation), position = position_jitter(0.15, 0.1), size = 2, colour = "black") + 
  theme(
    legend.position = "none"
  )
}

difference_in_seed_by_sample("GTAMix2")

map(mix_sample_codes,
    difference_in_seed_by_sample
    )
```
