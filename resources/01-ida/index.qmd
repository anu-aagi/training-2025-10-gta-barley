...
<!-- Not sure how to set the width of the box to be smaller than the IDA one -->
...
<b>🎯 **What is the efficacy and consistency of the ZoomAgri devices in detecting the correct barley varietal(s)?**</b>
...
## Goal of IDA: Data Description {.scrollable}
...
- Verify design 
- Identify observation unit <!--What is the smallest independent unit? Evaluation for each sample for a given device?  -->
...
## Don't be afraid to go digging 🕵️
...
```{r}
#| eval: false
# Add code to make classified seeds to predicted and observed seeds 

gta_input <- gta_tidy |> 
   mutate(predicted_seeds = round(percent/100 * classified_seeds),
         expected_seeds = round(percent_truth/100 * classified_seeds),
         sample_type = case_when(str_detect(sample_code, "Pure") ~ "Pure",
...
<!-- Go through these and talk about insights learned -->

**Check distributions - histogram of classified seeds and predicted seeds**


**Predicted percentages by device and sample**

```{r}

```

**Number of classified seeds by device and sample**
